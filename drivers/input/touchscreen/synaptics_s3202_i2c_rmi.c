...
/*****************************************************************/
/* OPPO 2013-09-22 ranfei Add begin for å¢žåŠ å¯¹å¯åŠ¨æ¨¡å¼çš„è¯†åˆ« */
...
static void vk_calculate_area(void)  //added by liujun
{
	...
	int margin_x = 110;		// Ã¿¸ö°´¼ü×óÓÒ¸÷ÁôµÄ×ø±êµã(Ïà¶ÔÓÚtp×ø±ê)
...
static void offset_correction(struct synaptics_ts_data *ts, int *pf0_x, int *pf0_y)
{
	...
	/*xÖáÎ»ÖÃÆ«ÒÆÐ£Õý*/
	if(f0_x <= syna_ts_data->snap_left)		/*½â¾ö´Ó´¥ÃþÆÁµÄ×î×ó±ßÍùÓÒ±ß»¬¶¯µÄÊ±ºòÃ»ÓÐ·´Ó¦µÄÎÊÌâ*/
	{
		f0_x = syna_ts_data->snap_left + 1;
	}
	else if(f0_x >= ts->max[0]-syna_ts_data->snap_right)	/*½â¾ö´Ó´¥ÃþÆÁµÄ×îÓÒ±ßÍù×ó±ß»¬¶¯µÄÊ±ºòÃ»ÓÐ·´Ó¦µÄÎÊÌâ*/
	{
...
static void synaptics_ts_work_func(struct work_struct *work)
{
	...
	if (ret)
	{
		...
		if(ts->need_hardware_reset > 3) {
			 /*¼ì²âµ½I2C´íÎó£¬Ó²¸´Î»I2C×ÜÏßÒÔ¼°×ÜÏßÉÏµÄËùÓÐÉè±¸*/
...
/* OPPO 2013-05-02 huanggd Add end*/			
		...
		/*±ØÐëÔÚ´Ë´¦ÍË³öwork£¬·ñÔòÓÐ¿ÉÄÜ½øÈëÏÂÃæµÄsynaptics_software_reset()£¬²¢Çåneed_hardware_reset£¬ÎÞ·¨Ó²¸´Î»*/
		goto work_func_end;
	}
...
/* OPPO 2013-05-02 huanggd Add end*/
	...
	else if (buf_status[1] & 0x04) /*0x04: Abs0 bit*/
	{
		...
		else
		{
			...
			else//if (finger_pressed == 0) /*Èç¹ûÃ»ÓÐÊÖÖ¸°´ÏÂ£¬ÉÏ±¨Ì§ÆðÊÂ¼þ*/
			{
				input_point_num = 0;
				input_mt_sync(ts->input_dev);
...
/* OPPO 2013-09-22 ranfei Add begin for åœ¨ATï¼ŒWLANå’ŒRFæ¨¡å¼ä¸æ³¨å†Œè§¦æ‘¸å± */
#ifdef CONFIG_VENDOR_EDIT
...
firmware_update:
    ...
    /*ranfei modify for N1 å‘å¸ƒä¼šä¸´æ—¶æ‰“å¼€è¿™å››ä¸ªå¼€å…³*/
